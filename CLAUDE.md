...
- **`pricing.jl`** -- Defines `Token` unit via Units.jl and `Price = USD/Mtoken`. `get_pricing(model)` reads `models.dev/api.json` on demand and returns `(input_price, output_price)` tuple, defaulting to zero.
